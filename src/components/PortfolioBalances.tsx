...
function PortfolioBalances() {
  ...
  let filteredTokens = tokenState.tokens.filter((token) => {
    return (
      ...
      !toBigNumber(token.balance).isZero()
    );
  });

  filteredTokens.sort((a, b) => {
    const priorBalance = toBigNumber(a.balance, { compression: a.decimals });
    const priorZilRate = a.isZil
      ? priorBalance
      : priorBalance.times(a.market_data.rate_zil);

    const nextBalance = toBigNumber(b.balance, { compression: b.decimals });
    const nextZilRate = b.isZil
      ? nextBalance
      : nextBalance.times(b.market_data.rate_zil);

    return priorZilRate.isLessThan(nextZilRate) ? 1 : -1;
  });

  var totalBalance = tokenState.tokens.reduce((sum, current) => {
    let balance = toBigNumber(current.balance, {
      compression: current.decimals,
    });
          ...
          <tbody>
            {filteredTokens.map((token, index) => {
              ...
              return (
                ...
                >
                  <td
                    className={`pl-4 pr-2 py-4 font-medium ${
                      index === 0 ? "rounded-tl-lg" : ""
                    } ${
                      index === filteredTokens.length - 1 ? "rounded-bl-lg" : ""
                    }`}
                  >
                    <Link href={`/tokens/${token.address.toLowerCase()}`}>
                      <a className="flex items-center">
                        ...
                        <span className="lg:font-normal ml-2 lg:text-gray-500">
                          {token.symbol}
                        </span>
                      </a>
                    </Link>
                  ...
                  <td className="px-2 py-2 font-normal text-right whitespace-nowrap">
                    <div>
                      <FlashChange value={fiatBalance}>
                        <span>
                          {currencyFormat(fiatBalance, selectedCurrency.symbol)}
                        </span>
                      </FlashChange>
                    </div>
                  ...
                  <td
                    className={`px-2 py-2 font-normal text-right ${
                      index === 0 ? "rounded-tr-lg" : ""
                    } ${
                      index === filteredTokens.length - 1 ? "rounded-br-lg" : ""
                    }`}
                  >
                    {moneyFormat(share, {
                      symbol: "",
