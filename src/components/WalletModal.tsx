...
const WalletModal = () => {
  ...
  const torchButtonRef = useRef(null);
  ...
  const connectTorch = async () => {
    ...
    if (typeof torch === "undefined") {
      console.log("Torch extension not installed");
      return;
    }

    const result = await torch.wallet.connect();

    if (result !== torch.wallet.isConnect) {
      console.log("Could not connect to Torch");
      return;
    }

    const walletAddress = torch.wallet.defaultAccount.bech32;
    let wallet: ConnectedWallet = {
      address: walletAddress,
      label: "",
      isDefault: accountState.wallets.length === 0,
          ...
          >
            <div className="relative bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 p-12 rounded-lg shadow-lg max-w-md mx-auto text-center">
              ...
              <Dialog.Description className="text-sm text-gray-500 dark:text-gray-400">
                Connect your wallet with ZilPay, Ledger, Avatar, Zeeves or enter
                an address.
              </Dialog.Description>
              ...
              {showDetail ? (
                <>
                  {showAvatarConnect && (
                    <ConnectAvatar onClose={() => setIsOpen(false)} />
                  )}
                ...
                <>
                  <div className="grid grid-cols-2 gap-3 my-8">
                    <button
                      ref={torchButtonRef}
                      className="font-bold bg-gray-200 hover:bg-gray-300 dark:bg-gray-800 dark:shadow py-6 rounded focus:outline-none flex flex-col items-center gap-3"
                      onClick={() => connectTorch()}
                    >
              ...
              <button
                ...
                className="focus:outline-none text-gray-500 text-sm mt-8"
              >
                Dismiss
              </button>
            </div>
