  ...
  return (
    <>
      ...
      <div className="bg-gradient-to-b from-cyan-500 to-blue-500 flex flex-col items-center px-12 py-20 rounded-xl">
        ...
        <p className="max-w-xl text-xl text-white text-center py-12">
          Introducing Wave SSN, a Zilliqa staking node powered by ZilStream and Carbon Labs. Support independent
          ecosystem developers by staking your ZIL at Wave.
        </p>
          ...
          <div className="grid grid-cols-3 gap-12">
            <div className="text-center p-4">
              <div className="text-4xl font-bold text-white mb-2">
                {cryptoFormat(waveOperator.stake_amount.shiftedBy(-12).toNumber())} ZIL
              </div>
            ...
            <div className="text-center p-4">
              <div className="text-4xl font-bold text-white mb-2">
                {numberFormat(waveOperator.comission.shiftedBy(-7).toNumber())}%
              </div>
            ...
            <div className="text-center p-4">
              <div className="text-4xl font-bold text-white mb-2">
                {cryptoFormat(waveOperator.stake_amount.shiftedBy(-12).toNumber())}
              </div>
              <div className="uppercase text-white opacity-70">Currently staked</div>
            </div>
          </div>
        ...
        <div className="bg-white bg-opacity-20 rounded-lg px-4 py-8 w-full mt-20 text-center flex flex-col items-center">
          ...
          accountState.selectedWallet.type === AccountType.ZilPay &&
          accountState.selectedWallet.isConnected ? (
            <div className="flex flex-col max-w-xl">
              <input
                className="rounded-lg bg-white bg-opacity-50 py-3 px-4 text-xl outline-none text-center text-white font-bold"
                type="number"
                placeholder="Enter amount"
              />
              <button className="rounded-lg bg-white bg-opacity-40 py-3 px-4 text-lg font-bold mt-1">Stake</button>
            </div>
          ) : (
            <button
              className="rounded-lg bg-white bg-opacity-40 py-3 px-4 text-lg font-bold mt-1"
              onClick={() => dispatch({ type: ModalActionTypes.OPEN_WALLET, payload: true })}
            >
              Connect
            </button>
          )}
