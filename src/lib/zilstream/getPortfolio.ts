...
import {
  ...
  xcadPoolBalanceBatchRequest,
  xcadZilPoolsBatchRequest,
  xcadZilTotalContributionsBatchRequest,
  xcadZilPoolBalanceBatchRequest,
  carbTokenPoolBalanceBatchRequest,
  carbPoolsBatchRequest,
  carbTotalContributionsBatchRequest,
  hunyBalanceBatchRequest,
  zilAllTokenPoolBalanceBatchRequest,
  zilAllPoolsBatchRequest,
  zilAllTotalContributionsBatchRequest,
} from "utils/batch";
import { Network } from "utils/network";

export default async function getPortfolioState(
  ...
  tokens.forEach((token) => {
    if (token.address.startsWith("0x")) {
    } else if (token.address === ZIL_ADDRESS) {
      batchRequests.push(balanceBatchRequest(token, walletAddress));
      batchRequests.push(
        carbTokenPoolBalanceBatchRequest(token, walletAddress)
      );
    } else {
      ...
      batchRequests.push(
        zilAllTokenPoolBalanceBatchRequest(token, walletAddress)
      );
    }
  });
